...
(function (global) {
  ...
  class BranchNode {
    ...
    draw(time) {
      this.tree.isDrawingLeaf = false;
      this.tree._leafDrawDepth = 0;
      this.tree.isDrawingBranch = true;
      push();
      
      let swayAngle = 0;
      if (this.tree.windStrength > 0 && this.level > 0) {
        let bias = (this.tree.windBias !== undefined) ? this.tree.windBias : 0;
        swayAngle = (sin(time + this.depth * 0.4) * (this.tree.windStrength * 0.006) + bias * 0.008) * (this.tree.maxDepth - this.depth + 1);
      }
      ...
      this.tree.isDrawingBranch = false;
  ...
  class FractalBranchTree {
    ...
    setBranchGradient(depth, branchLen) {
      this.isDrawingBranch = true;
      this.isDrawingLeaf = false;
      this._leafDrawDepth = 0;
    ...
    setLeafFillColor(branchId) {
      ...
      switch(this.leafType) {
        case 'sakura':
          if (hashColor < 0.33) {
            fill(253, 164, 186); // Sakura Pink
          } else if (hashColor < 0.66) {
            fill(244, 114, 182); // Soft Magenta
          } else {
            fill(252, 231, 243); // Pearl White-Pink
          }
          break;

        case 'autumn':
          if (hashColor < 0.25) {
            fill(220, 38, 38);  // 1. Đỏ tươi rực rỡ (Crimson Red)
          } else if (hashColor < 0.50) {
            fill(239, 68, 68);  // 2. Đỏ hoa hiên rực cháy (Fiery Scarlet Red)
          } else if (hashColor < 0.75) {
            fill(249, 115, 22); // 3. Cam đỏ chuyển giao (Fiery Orange-Red)
          } else {
            fill(185, 28, 28);  // 4. Đỏ mận chín thẫm (Deep Ruby Wine Red)
          }
          break;

        case 'ginkgo':
          if (hashColor < 0.35) {
            fill(250, 204, 21); // Bright Canary Yellow
          } else if (hashColor < 0.70) {
            fill(234, 179, 8); // Sun Gold
          } else {
            fill(245, 158, 11); // Warm Honey
          }
          break;

        case 'wisteria':
          if (hashColor < 0.33) {
            fill(192, 132, 252); // Pastel Lavender
          } else if (hashColor < 0.66) {
            fill(168, 85, 247); // Royal Violet
          } else {
            fill(232, 121, 249); // Soft Orchid
          }
          break;
        ...
        case 'frost':
          if (this.isDark === false) {
            if (hashColor < 0.35) {
              fill(2, 132, 199); // Cobalt Glacial Azure (#0284c7)
            } else if (hashColor < 0.70) {
              fill(14, 165, 233); // Vibrant Glacial Sky (#0ea5e9)
            } else {
              fill(56, 189, 248); // Crisp Ice Cyan (#38bdf8)
            }
          } else {
            if (hashColor < 0.35) {
              fill(56, 189, 248); // Ice Cyan (#38bdf8)
            } else if (hashColor < 0.70) {
              fill(125, 211, 252); // Diamond Sky (#7dd3fc)
            } else {
              fill(224, 242, 254); // Frozen Crystal (#e0f2fe)
            }
          }
          break;
        ...
        case 'sunset':
          if (hashColor < 0.33) {
            fill(244, 63, 94); // Coral Red
          } else if (hashColor < 0.66) {
            fill(251, 146, 60); // Fiery Peach
          } else {
            fill(251, 191, 36); // Golden Flame
          }
          break;

        case 'midnight':
          if (hashColor < 0.35) {
            fill(99, 102, 241); // Celestial Indigo
          } else if (hashColor < 0.70) {
            fill(59, 130, 246); // Sapphire Blue
          } else {
            fill(129, 140, 248); // Neon Violet-Blue
          }
          break;

        case 'eucalyptus':
          if (hashColor < 0.35) {
            fill(146, 185, 165); // Glaucous Sage Silver Green (Xanh bạc sương mù)
          } else if (hashColor < 0.70) {
            fill(168, 202, 184); // Powdered Eucalyptus White-Green (Xanh phủ phấn trắng)
          } else {
            fill(122, 160, 142); // Dusty Eucalyptus Teal (Xanh ngọc phai bạc thẫm)
          }
          break;

        case 'golden':
          if (hashColor < 0.35) {
            fill(254, 240, 138); // Kim Sa Hoàng Kim
          } else if (hashColor < 0.70) {
            fill(250, 204, 21);  // Vàng Hoàng Gia
          } else {
            fill(202, 138, 4);   // Hổ Phách Vàng Đồng
          }
          break;

        case 'ruby':
          if (hashColor < 0.35) {
            fill(251, 113, 133); // Hồng Ngọc Sáng
          } else if (hashColor < 0.70) {
            fill(244, 63, 94);   // Huyết Ngọc
          } else {
            fill(190, 18, 60);   // Hồng Ngọc Thẫm
          }
          break;

        case 'shadow':
          if (hashColor < 0.35) {
            fill(148, 163, 184); // Xám Bạc Hư Không
          } else if (hashColor < 0.70) {
            fill(71, 85, 105);   // Hắc Thạch
          } else {
            fill(30, 41, 59);    // Huyền Dạ Đen Tuyền
          }
          break;
        ...
        default:
          if (hashColor < 0.35) {
            fill(52, 211, 153); // Emerald Green
          } else if (hashColor < 0.70) {
            fill(110, 231, 183); // Mint Green
          } else {
            fill(5, 150, 105); // Deep Forest Green
          }
          break;
      }
    }
